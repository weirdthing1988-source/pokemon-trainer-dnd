...
function damageDice(mon, m) { if (m.category === 'status') return 0; const r = rank(mon.base[offensiveKey(m)]); return Math.ceil(r / 2) + milestoneCount(mon.level) * eligiblePB(mon); }
...
function monBadge(mon, size = 'small') {
  if (mon?.sprite) return `<div class="mon-badge image ${size}"><img src="${escapeAttr(mon.sprite)}" alt="" loading="lazy"></div>`;
...
function renderEditor() {
  ...
  pokemonEditor.innerHTML = `
    <div class="editor-head">
      ...
      <div class="editor-actions"><button id="switchBtn" class="button primary" ${active || fainted ? 'disabled' : ''}>${active ? 'Currently Active' : fainted ? 'Cannot Switch In' : 'Switch In'}</button></div>
    </div>
...
function statCard(label, key, mon) {
  const base = Number(mon.base[key]) || 0, r = rank(base), mod = statModFromRank(r);
  const extra = key === 'hp' ? `Max ${maxHpFor(mon)}` : `Mod ${sign(mod)}`;
  return `<div class="stat-card"><label>${label} Base</label><input class="base-stat" data-stat="${key}" type="number" min="1" max="255" value="${base}"><div class="stat-derived"><span>Rank ${r}</span><span>${extra}</span></div></div>`;
}
...
function moveCard(mon, m, idx, active, fainted) {
  ...
  const preview = m.category === 'status' ? 'Status / utility — resolve the effect manually.' : m.sureHit ? `<strong>Sure Hit</strong> · ${dice}${m.die}` : `<strong>${sign(attackBonus)} To Hit</strong> · ${dice}${m.die} · Accuracy ${aScore}`;
  return `<div class="move-card" data-move-index="${idx}"><div class="move-top"><div class="move-name">${escapeHtml(m.name)}</div><span class="move-tag">${escapeHtml(m.type)}</span></div><div class="move-tags"><span class="move-tag">${m.category.toUpperCase()}</span>${m.power ? `<span class="move-tag">Power ${m.power}</span>` : ''}${m.sureHit ? '<span class="move-tag">SURE HIT</span>' : ''}</div>${m.effect ? `<p class="move-effect">${escapeHtml(m.effect)}</p>` : ''}<div class="move-preview">${preview}</div><button class="button primary use-move" ${!active || fainted ? 'disabled' : ''}>Use ${escapeHtml(m.name)}</button></div>`;
}
...
async function loadPokemonIndex() {
  ...
  try {
    ...
    runtime.pokemonIndex = (data.results || []).map((item, idx) => ({ name: item.name, url: item.url, id: pokemonIdFromUrl(item.url) || idx + 1 }));
